...
export default class SubjectDetailsAdmin extends Component {
  render() {
    ...
    return (
        <div className="SubjectDetailsDiv card text-center">
            <div className="card-header"><h2>{subject.name}</h2></div>
            <div className="card-body">
                <div style={{ whiteSpace: "pre-wrap" }}>{subject.description}</div><br />
                <ul className="list-group list-group-flush">
                    <li className="list-group-item"><span className="badge badge-dark">Vyučující:</span> {subject.teacher}</li>
            ...
            <div className="card-footer">
                <div className="btn-group">
                    ...
                    <button className="btn btn-danger" onClick={() => this.props.remove(subject.id)}>Smazat</button>
                </div>
            </div>
        </div>
        );
